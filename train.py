...
import torch
import numpy as np
from model.biocrepe import BioCrepe, metrics, confusion_matrix, roc
...
if __name__ == '__main__':
    ...
    if cuda:
        model = BioCrepe().cuda()  # not finished yet
    else:
    ...
    logging.info("Model compiled: {} parameters in total.".format(sum([len(layer.view(-1)) for layer in model.parameters()])))
